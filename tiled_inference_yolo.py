...
def collect_predictions_img_lvl(task: str,predictions: list, patches: list, device: torch.device, patch_output_dir: tuple[str, None]) -> torch.Tensor:
    if task == "detect":
    ...
    for i in range(coords.shape[0]):
        if boxes:
            ...
            cv2.circle(img=img_arr, center=(int(round(coords[i, 0].item())), int(round(coords[i, 1].item()))),
                       radius=PT_VIS_CIRCLE_RADIUS, color=(0, 255, 0), thickness=-1)
...
def run_tiled_inference(model_file: str, task: str, radii: dict, class_ids: list, imgs_dir: str, img_files_ext: str, tiling_dir: str, 
                        patch_dims: dict, patch_overlap: float, patch_quality: int, vis_dir: str, det_dir: str, vis_prob: float, 
                        save_pre_output: bool = False, iou_thresh: float = None, dor_thresh: float = None, max_offset: int = 1, 
    ...
    for fn in tqdm(img_fns, total=len(img_fns)):
        ...
        visualize = (random.randint(0, 1000) / 1000 <= vis_prob) or pre_nms >= PLOTTING_THRESH
        ...
        if task == "detect":
            ...
            idxs = loc_nms(preds=coords_shifted, scores=conf.squeeze(1), radii=radii_t, dor_thresh=dor_thresh)
